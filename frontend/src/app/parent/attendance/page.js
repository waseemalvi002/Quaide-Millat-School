  ...
  return (
    <DashboardLayout>
      <div style={{ padding: '24px' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '4px' }}>Child&apos;s Attendance</h1>
        <p style={{ color: '#64748b', marginBottom: '24px' }}>Ali Khan (QM-05-001) — {((present / working) * 100).toFixed(1)}% attendance</p>
        <div className="card">
          <div className="table-wrapper">
            <table className="table">
              ...
              <tbody>
                {DEMO.map((d, i) => (
                  <tr key={i}><td style={{ fontWeight: 600 }}>{d.date}</td><td>{d.day}</td><td><span className={`badge ${statusBadge[d.status]}`}>{d.status}</span></td></tr>
                ))}
              </tbody>
            </table>
          </div>
